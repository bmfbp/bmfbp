...
(defun make-bounding-boxes-for-rectangles ()
  (let ((rect-list (prove-all '((rect ?id)
                                ...
                                (geometry-h ?id ?H))
                              no-bindings)))
    (format *error-output* "~&rect-list=~S" rect-list)
    (mapc #'(lambda (alist)
...
(defun make-bounding-boxes-for-speech-bubbles ()
  (let ((speech-list (prove-all '((speech ?id)
                                ...
                                (geometry-h ?id ?H))
                              no-bindings)))
    (format *error-output* "~&speech-list=~S" speech-list)
    (mapc #'(lambda (alist)
              (create-rect-bb alist))
          speech-list)))
...
(defun make-bounding-boxes-for-text ()
  (let ((text-list (prove-all '((id ?id)
                                (geometry-top-y ?id ?Y)
                                (geometry-center-x ?id ?X)
                                (geometry-w ?id ?W)
                                (geometry-h ?id ?H))
                              no-bindings)))
    (format *error-output* "~&text-list=~S" text-list)
    (mapc #'(lambda (alist)
...
(defun make-bounding-boxes-for-ellipses ()
  (let ((ellipse-list (prove-all '((id ?id)
                                ...
                                (geometry-h ?id ?H))
                              no-bindings)))
    (format *error-output* "~&ellipse-list=~S" ellipse-list)
    (mapc #'(lambda (alist)
