...
(defmethod compiler-part-name ((self compiler-part))
  "FRONT-END")

(defmethod compiler-part-initially ((self compiler-part))
  (set (state self) :idle))

(defmethod compiler-part-run ((self compiler-part) e)
  (ecase (state self)
    (:idle
...
(defun main (svg-filename)
  (let ((command-svg-to-lisp "~/bin/hs_vsh_drawio_to_fb"))
    (let ((str (with-output-to-string (s)
                 (system:call-system-showing-output (format nil "~a <~a" program svg-filename) :output-stream s))))
