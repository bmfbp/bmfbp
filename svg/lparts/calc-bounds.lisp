...
#+nil(defun exactly-one (lis)
  (assert (= 1 (length lis)))
  (car lis))

#+nil(defparameter *id* nil)

#+nil(defun create-rect-bb (id)
  (let ((*id* id))
    (let ((x (prove-all '((rect ?id) 
          (y (exactly-one (paiprolog:prolog-collect (?top)  (paiprolog::lisp ?id *id*) (geometry-top-y ?id ?top))))
          (w (exactly-one (paiprolog:prolog-collect (?w)    (paiprolog::lisp ?id *id*) (geometry-w ?id ?w))))
          (h (exactly-one (paiprolog:prolog-collect (?h)    (paiprolog::lisp ?id *id*) (geometry-h ?id ?h)))))
      (format *standard-output* "~&*id*=~S x=~S y=~S w=~S h=~S~%" *id* x y w h)
      (let ((right (+ x w))
            (bottom (+ y h)))
        (paiprolog::add-clause `(bounding-box-left ,id ,x) :asserta t)
        (paiprolog::add-clause `(bounding-box-top ,id ,y) :asserta t)
        (paiprolog::add-clause `(bounding-box-right ,id ,right) :asserta t)
        (paiprolog::add-clause `(bounding-box-bottom ,id ,bottom) :asserta t))))))))
