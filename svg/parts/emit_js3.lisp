...
(defun emit-parts (&key self)
  (labels ((emit-wires-for-pin (pin-id pinlist)
           ...
           (emit-pins (properties)
             ...
             (format *standard-output* "      \"inOrdering\" : ~S,~%" (as-json-map (gethash :ins-enumeration properties)))
             (format *standard-output* "      \"outCount\" : ~A,~%" (length (gethash :outs-enumeration properties)))
             (format *standard-output* "      \"outOrdering\" : ~S,~%" (as-json-map (gethash :ins-enumeration properties)))
             (emit-ins properties)
             (emit-outs properties))
