    ...
    (let ((fact (car lfact)))
      (if (not (or (= 3 (length fact)) (= 2 (length fact))))
          (progn
            ...
            (ecase (length fact)
              (4 (format stream "~a(~a,~a,~a,~a).~%" (first fact) (second fact) (third fact) (fourth fact) (fifth fact)))
              (5 (format stream "~a(~a,~a,~a,~a,~a).~%" (first fact) (second fact) (third fact) (fourth fact) (fifth fact)
                         (sixth fact)))
              (6 (format stream "~a(~a,~a,~a,~a,~a,~a).~%" (first fact) (second fact) (third fact) (fourth fact) (fifth fact)
                         (sixth fact) (seventh fact)))
              (7 (format stream "~a(~a,~a,~a,~a,~a,~a,~a).~%" (first fact) (second fact) (third fact) (fourth fact) (fifth fact)
                         (sixth fact) (seventh fact) (eighth fact)))
              (8 (format stream "~a(~a,~a,~a,~a,~a,~a,~a,~a).~%" (first fact) (second fact) (third fact) (fourth fact) (fifth fact)
