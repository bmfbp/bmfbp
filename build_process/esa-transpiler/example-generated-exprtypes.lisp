...
(defmethod initialize-instance :after ((self statement) &key &allow-other-keys)
  (setf (stack-dsl::%type-list self) '("returnValueStatement" "returnFalseStatement" "returnTrueStatement" "callExternalStatement" "callInternalStatement" "exitWhenStatement" "loopStatement" "ifStatement" "createStatement" "setStatement" "exitMapStatement" "mapStatement" "letStatement")))
...
(let ((r (and
(let ((in-eq (eq (nth 0 wm) (stack-dsl::%stack (input-returnFalseStatement self))))
      (out-eq (eq (nth 1 wm) (stack-dsl::%stack (output-returnFalseStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 2 wm) (stack-dsl::%stack (input-filler self))))
      (out-eq (eq (nth 3 wm) (stack-dsl::%stack (output-filler self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 4 wm) (stack-dsl::%stack (input-mapStatement self))))
      (out-eq (eq (nth 5 wm) (stack-dsl::%stack (output-mapStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 6 wm) (stack-dsl::%stack (input-fieldMap self))))
      (out-eq (eq (nth 7 wm) (stack-dsl::%stack (output-fieldMap self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 8 wm) (stack-dsl::%stack (input-returnType self))))
      (out-eq (eq (nth 9 wm) (stack-dsl::%stack (output-returnType self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 10 wm) (stack-dsl::%stack (input-returnValueStatement self))))
      (out-eq (eq (nth 11 wm) (stack-dsl::%stack (output-returnValueStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 12 wm) (stack-dsl::%stack (input-declarationMethodOrScript self))))
      (out-eq (eq (nth 13 wm) (stack-dsl::%stack (output-declarationMethodOrScript self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 14 wm) (stack-dsl::%stack (input-actualParameterList self))))
      (out-eq (eq (nth 15 wm) (stack-dsl::%stack (output-actualParameterList self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 16 wm) (stack-dsl::%stack (input-callExternalStatement self))))
      (out-eq (eq (nth 17 wm) (stack-dsl::%stack (output-callExternalStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 18 wm) (stack-dsl::%stack (input-expression self))))
      (out-eq (eq (nth 19 wm) (stack-dsl::%stack (output-expression self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 20 wm) (stack-dsl::%stack (input-elsePart self))))
      (out-eq (eq (nth 21 wm) (stack-dsl::%stack (output-elsePart self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 22 wm) (stack-dsl::%stack (input-whenDeclarations self))))
      (out-eq (eq (nth 23 wm) (stack-dsl::%stack (output-whenDeclarations self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 24 wm) (stack-dsl::%stack (input-esaKind self))))
      (out-eq (eq (nth 25 wm) (stack-dsl::%stack (output-esaKind self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 26 wm) (stack-dsl::%stack (input-situations self))))
      (out-eq (eq (nth 27 wm) (stack-dsl::%stack (output-situations self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 28 wm) (stack-dsl::%stack (input-situationReferenceList self))))
      (out-eq (eq (nth 29 wm) (stack-dsl::%stack (output-situationReferenceList self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 30 wm) (stack-dsl::%stack (input-whenDeclaration self))))
  ...
  (and in-eq out-eq))
(let ((in-eq (eq (nth 32 wm) (stack-dsl::%stack (input-formalList self))))
      (out-eq (eq (nth 33 wm) (stack-dsl::%stack (output-formalList self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 34 wm) (stack-dsl::%stack (input-methodDeclarationsAndScriptDeclarations self))))
      (out-eq (eq (nth 35 wm) (stack-dsl::%stack (output-methodDeclarationsAndScriptDeclarations self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 36 wm) (stack-dsl::%stack (input-lval self))))
      (out-eq (eq (nth 37 wm) (stack-dsl::%stack (output-lval self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 38 wm) (stack-dsl::%stack (input-callInternalStatement self))))
      (out-eq (eq (nth 39 wm) (stack-dsl::%stack (output-callInternalStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 40 wm) (stack-dsl::%stack (input-setStatement self))))
      (out-eq (eq (nth 41 wm) (stack-dsl::%stack (output-setStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 42 wm) (stack-dsl::%stack (input-indirectionKind self))))
      (out-eq (eq (nth 43 wm) (stack-dsl::%stack (output-indirectionKind self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 44 wm) (stack-dsl::%stack (input-methodName self))))
      (out-eq (eq (nth 45 wm) (stack-dsl::%stack (output-methodName self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 46 wm) (stack-dsl::%stack (input-exitMapStatement self))))
      (out-eq (eq (nth 47 wm) (stack-dsl::%stack (output-exitMapStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 48 wm) (stack-dsl::%stack (input-statement self))))
      (out-eq (eq (nth 49 wm) (stack-dsl::%stack (output-statement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 50 wm) (stack-dsl::%stack (input-typeName self))))
      (out-eq (eq (nth 51 wm) (stack-dsl::%stack (output-typeName self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 52 wm) (stack-dsl::%stack (input-externalMethod self))))
      (out-eq (eq (nth 53 wm) (stack-dsl::%stack (output-externalMethod self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 54 wm) (stack-dsl::%stack (input-scriptDeclaration self))))
      (out-eq (eq (nth 55 wm) (stack-dsl::%stack (output-scriptDeclaration self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 56 wm) (stack-dsl::%stack (input-fkind self))))
      (out-eq (eq (nth 57 wm) (stack-dsl::%stack (output-fkind self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 58 wm) (stack-dsl::%stack (input-exitWhenStatement self))))
      (out-eq (eq (nth 59 wm) (stack-dsl::%stack (output-exitWhenStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 60 wm) (stack-dsl::%stack (input-internalMethod self))))
      (out-eq (eq (nth 61 wm) (stack-dsl::%stack (output-internalMethod self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 62 wm) (stack-dsl::%stack (input-functionReference self))))
      (out-eq (eq (nth 63 wm) (stack-dsl::%stack (output-functionReference self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 64 wm) (stack-dsl::%stack (input-thenPart self))))
      (out-eq (eq (nth 65 wm) (stack-dsl::%stack (output-thenPart self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 66 wm) (stack-dsl::%stack (input-situationReferenceName self))))
      (out-eq (eq (nth 67 wm) (stack-dsl::%stack (output-situationReferenceName self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 68 wm) (stack-dsl::%stack (input-esaprogram self))))
      (out-eq (eq (nth 69 wm) (stack-dsl::%stack (output-esaprogram self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 70 wm) (stack-dsl::%stack (input-varName self))))
      (out-eq (eq (nth 71 wm) (stack-dsl::%stack (output-varName self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 72 wm) (stack-dsl::%stack (input-object self))))
      (out-eq (eq (nth 73 wm) (stack-dsl::%stack (output-object self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 74 wm) (stack-dsl::%stack (input-typeDecl self))))
      (out-eq (eq (nth 75 wm) (stack-dsl::%stack (output-typeDecl self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 76 wm) (stack-dsl::%stack (input-returnKind self))))
      (out-eq (eq (nth 77 wm) (stack-dsl::%stack (output-returnKind self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 78 wm) (stack-dsl::%stack (input-methodDeclaration self))))
      (out-eq (eq (nth 79 wm) (stack-dsl::%stack (output-methodDeclaration self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 80 wm) (stack-dsl::%stack (input-name self))))
      (out-eq (eq (nth 81 wm) (stack-dsl::%stack (output-name self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 82 wm) (stack-dsl::%stack (input-letStatement self))))
      (out-eq (eq (nth 83 wm) (stack-dsl::%stack (output-letStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 84 wm) (stack-dsl::%stack (input-loopStatement self))))
      (out-eq (eq (nth 85 wm) (stack-dsl::%stack (output-loopStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 86 wm) (stack-dsl::%stack (input-returnTrueStatement self))))
  ...
  (and in-eq out-eq))
(let ((in-eq (eq (nth 88 wm) (stack-dsl::%stack (input-implementation self))))
      (out-eq (eq (nth 89 wm) (stack-dsl::%stack (output-implementation self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 90 wm) (stack-dsl::%stack (input-field self))))
      (out-eq (eq (nth 91 wm) (stack-dsl::%stack (output-field self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 92 wm) (stack-dsl::%stack (input-createStatement self))))
      (out-eq (eq (nth 93 wm) (stack-dsl::%stack (output-createStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 94 wm) (stack-dsl::%stack (input-methodsTable self))))
      (out-eq (eq (nth 95 wm) (stack-dsl::%stack (output-methodsTable self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 96 wm) (stack-dsl::%stack (input-classes self))))
      (out-eq (eq (nth 97 wm) (stack-dsl::%stack (output-classes self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 98 wm) (stack-dsl::%stack (input-esaclass self))))
      (out-eq (eq (nth 99 wm) (stack-dsl::%stack (output-esaclass self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 100 wm) (stack-dsl::%stack (input-ekind self))))
      (out-eq (eq (nth 101 wm) (stack-dsl::%stack (output-ekind self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 102 wm) (stack-dsl::%stack (input-ifStatement self))))
      (out-eq (eq (nth 103 wm) (stack-dsl::%stack (output-ifStatement self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 104 wm) (stack-dsl::%stack (input-typeDecls self))))
      (out-eq (eq (nth 105 wm) (stack-dsl::%stack (output-typeDecls self)))))
  (and in-eq out-eq))
(let ((in-eq (eq (nth 106 wm) (stack-dsl::%stack (input-situationDefinition self))))
      (out-eq (eq (nth 107 wm) (stack-dsl::%stack (output-situationDefinition self)))))
  (and in-eq out-eq))
)))
