...
(%field-type-fieldName :accessor %field-type-fieldName :initform "fieldName")
...
(defclass fieldName (stack-dsl::%string) () (:default-initargs :%type "fieldName"))
(defclass fieldName-stack (stack-dsl::%typed-stack) ())
(defmethod initialize-instance :after ((self fieldName-stack) &key &allow-other-keys)
  (setf (stack-dsl::%element-type self) "fieldName"))
...
(stack-dsl::%ensure-existence 'fieldName)
...
(input-fieldName :accessor input-fieldName :initform (make-instance 'fieldName-stack))
(output-fieldName :accessor output-fieldName :initform (make-instance 'fieldName-stack))
))
...
(input-field self)
(output-field self)
(input-fieldName self)
(output-fieldName self)
)))
...
(eq (nth 10 wm) (input-empty self))
(eq (nth 11 wm) (output-empty self))
(eq (nth 12 wm) (input-nameList self))
(eq (nth 13 wm) (output-nameList self))
(eq (nth 14 wm) (input-fieldMap self))
(eq (nth 15 wm) (output-fieldMap self))
(eq (nth 16 wm) (input-field self))
(eq (nth 17 wm) (output-field self))
(eq (nth 18 wm) (input-fieldName self))
(eq (nth 19 wm) (output-fieldName self))
))
