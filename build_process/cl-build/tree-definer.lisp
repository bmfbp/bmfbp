...
;;
;; convert JSON graph into (internal) data structure
;;  as shown in diagram "data-structures.drawio" tab "definition"
;;
;; function "react" is the mainline
;;
;;;;;;;;;;;;;
#|

    input == JSON containing 2 types of objects:
    1. "itemKind" : "leaf" ...
    2. "itemKind" : "graph" ...

    1.  contains a part name and a filename
    2.  cotains a part name and a graph, see below
...
(defclass part-namer (tree-definer)
...
(incf (counter self))
(format *standard-output* "~&~tree-definer gets ~a ~s~%" (counter self) (@pin self e) (@data self e))
  (ecase (@pin self e)
    (:json-script
     (let ((json-script (@data self e)))
       ;; convert JSON to lisp alist
       (let ((alist-script (with-input-from-string (s json-script) (json:decode-json s))))
         ;; walk the alist
         (perform-tree-defining alist-script)
         ;; send the tree root downstream (tree remains in memory)
         (let ((top-name (find-top-schematic-name alist-script)))
           (@send self :function top-name))))

(defun perform-tree-defining (alist-script)
  (dolist (alist-item alist-script)
    (let ((item-kind (cdr (assoc :item-kind item))))
      (ecase item-kind
        (:leaf
         (tree-definer-file (cdr (assoc :filename item))))
        (:graph
         (tree-definer-graph (cdr (assoc :graph item))))))))

(defun tree-definer-file (filename)
  (tree-definer filename))
...
(defun tree-definer-graph (graph-alist)
...
(defmethod process-sources (self sources wire parts-hash)
 ...
 (let ((proceed (validate-sources sources parts-hash)))
   (if proceed
       ()
     (@send self "tree-definer quit due to source error"))))
...
(defmethod process-receivers (receivers wire parts-hash)
 ...
 (let ((proceed (validate-receivers receivers parts-hash)))
   (if proceed
       ()
     (@send self "tree-definer quit due to receiver error"))))
