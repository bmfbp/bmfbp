...
(defclass strcat1 (e/part:code) 
  ((state :accessor state)
   (x :accessor x)
   (y :accessor y)))

(defmethod e/part:busy-p ((self strcat1))
  (call-next-method))

(defmethod e/part:clone ((self strcat1))
  (call-next-method))

(defmethod e/part:first-time ((self strcat1))
  (setf (state self) :idle)
  (setf (x self) "")
  (setf (y self) ""))

(defmethod e/part:react ((self strcat1) (e e/event:event))
  (ecase (state self)
    (:idle
...
(defmethod send-result ((self strcat1))
  (let ((result (concatenate 'string (x self) (y self))))
    (format *standard-output* "~&strcat1 -> ~a~%" result)
